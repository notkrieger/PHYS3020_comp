...
# inputs:
    ...
    # lastU - internal energy of the previous state, used to speed up calc for nextU
# returns:
...
def find_multiplicity(state):
    ...
    for dipole in state:
        if dipole > 0:
            up_count += 1
    #print("up percent: " + str(up_count/N))
    return float(comb(N, up_count))


def model(temp): # one dimensional Ising Model
    # variables
    total_steps = 1000 * N # is there some way to make a distribution
    # so all dipoles definitely get selected?
    ...
    for i in range(total_steps):
        Ss[i] = np.log(find_multiplicity(spins))
        #print(Us[i])
    ...
    S = np.mean(Ss[-int(total_steps / 10)]) # entropy S = k ln(g), do I use S = -k * sum(P(i) * ln (P(i))?
